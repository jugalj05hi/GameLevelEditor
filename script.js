...
// var rArray = document.querySelector("#reverse");
...
function generate() {
    let width, height;
    if (document.getElementById("canvaswidth").value != 0)
    ...
    if (document.getElementById("tilesize").value) 
...
function setTileSize(size) {
    tileSize = size;
    document.getElementById('tileset-selector').style.width = tileSize;
    document.getElementById('tileset-selector').style.height = tileSize;
    setTileSheetMap();
}

function uploadImage() {
    tilesetImage.src = tempImg;
...
function reverseLookUp(clicked) {
    // var clicked = getCoords(event);
    // console.log(clicked);
    let x = clicked[0];
    let y = clicked[1];
...
function deleteTile(coords) {
    let coordinates = [...coords];
    let x = coordinates[0];
    let y = coordinates[1];
    eArray[x][y] = 0;
}
function printExportArray() {
    // console.log(eArray);
}
...
function getCoords(e) {
    ...
    // console.log(array[Math.floor(mouseX / 32)][Math.floor(mouseY / 32)])
...
function setTileSheetMap() {
    let cols = Math.floor(tilesetImage.height / tileSize);
    let rows = Math.floor(tilesetImage.width / tileSize);
    let counter = 1;
    array = Array.from(Array(rows), () => new Array(cols));
    ...
    for (let i = 0; i < cols; i++) {
        ...
        for (let j = 0; j < rows; j++) {
            ...
            array[j][i] = counter;
            tileMap.set(counter, [j, i]);
            counter += 1;
        }
    }
    // let height = Math.floor(tilesetImage.height / 32);
    // let width = Math.floor(tilesetImage.width / 32);
    // let counter = 0;
    ...
    //     }
    // }
    // console.log("TileSheetMap", array);
}
function saveFile() {
    let cols = Math.floor(canvas.height / tileSize);
    let rows = Math.floor(canvas.width / tileSize);
...
function setTileMap() {
    ...
    let cols = Math.floor(canvas.height / tileSize);
    let rows = Math.floor(canvas.width / tileSize);
    eArray = Array.from(Array(rows), () => new Array(cols));
    // eArray = new Array(cols)
    for (let i = 0; i < cols; i++) {
        for (let j = 0; j < rows; j++) {
            // console.log("TileMap", j, i);
            eArray[j][i] = 0;
        }
    }
}
...
document.querySelector("#fileUpload").addEventListener('input', function () {
    if (this.files && this.files[0]) {
        ...
        img.onload = () => {
            ...
            setTileMap();
            draw();
            setLayer(0);  // no longer needed, free memory
        }

        tempImg = URL.createObjectURL(this.files[0]); // set src to blob url
    }
});
